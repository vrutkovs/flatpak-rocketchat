...
build: build-electron-base
	flatpak-builder --force-clean --ccache --require-changes --repo=repo \
		--subject="Nightly build of Rocketchat, `date`" \
		${EXPORT_ARGS} app chat.rocket.App.json

prune:
	flatpak build-update-repo --prune --prune-depth=20 repo

install-repo:
	flatpak --user remote-add --if-not-exists --no-gpg-verify nightly-rocket ./repo
	flatpak --user -v install nightly-rocket chat.rocket.App || true
